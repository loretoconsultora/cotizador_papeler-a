...
export async function fetchQuoteForPdf(supabase: SupabaseClient, quoteId: string) {
  const { data: quote, error: quoteError } = await supabase
    .from("quotes")
    .select(
      "id, folio, status, client_name_snapshot, company_name_snapshot, phone_snapshot, email_snapshot, address_snapshot, purchase_condition, payment_method, subtotal, discount_total, iva_pct, iva_total, grand_total, rfc_snapshot, tax_regime_snapshot, cfdi_use_snapshot, carrier_name, delivery_type, delivery_address, created_at"
    )
    .eq("id", quoteId)
    .single();
